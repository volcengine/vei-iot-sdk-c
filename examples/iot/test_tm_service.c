...
#define TAG "test_tm_service_call.c"
    ...
    switch (recv->type) {
        case IOT_TM_RECV_SERVICE_CALL: {
            ...
            if (strcmp("test_service_syn", recv->data.service_call.identifier) == 0) {
                ...
                iot_tm_msg_service_call_reply_init(&reply, recv->data.service_call.module_key, recv->data.service_call.identifier, recv->data.service_call.topic_uuid,
                                                        recv->data.service_call.msg_id, 0);
                iot_tm_msg_service_call_reply_set_prams_json_str(reply, "{\"test_num_out\":22}");
                service_reply_msg.data.service_call_reply = reply;
                iot_tm_send(dm, &service_reply_msg);
                ...
                iot_tm_msg_service_call_reply_init(&reply, recv->data.service_call.module_key, recv->data.service_call.identifier, recv->data.service_call.topic_uuid,
                                                        recv->data.service_call.msg_id, 0);
                iot_tm_msg_service_call_reply_set_prams_json_str(reply, "{\"test_string_out\":\"asdas\"}");
                service_reply_msg.data.service_call_reply = reply;
                iot_tm_send(dm, &service_reply_msg);
                iot_tm_msg_service_call_reply_free(reply);
            }
